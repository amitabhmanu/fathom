...
describe("postToolUse hook shim (compiled, real subprocess)", () => {
  it("returns the minimal no-op shape and logs the fixture raw, unchanged", async () => {
    ...
    const fixture = { ...(loadFixture("postToolUse.edit.json") as Record<string, unknown>), cwd: daemon.endpoint.projectRoot };

    const result = await runCompiledHookShim("postToolUse", fixture, {
      ...process.env,
    ...
    expect(latest?.event_name).toBe("PostToolUse");

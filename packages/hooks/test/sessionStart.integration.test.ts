...
describe("sessionStart hook shim (compiled, real subprocess)", () => {
  it("returns the minimal no-op shape and logs the fixture raw, unchanged", async () => {
    ...
    const fixture = { ...(loadFixture("sessionStart.startup.json") as Record<string, unknown>), cwd: daemon.endpoint.projectRoot };

    const result = await runCompiledHookShim("sessionStart", fixture, {
      ...process.env,
    ...
    expect(latest?.event_name).toBe("SessionStart");

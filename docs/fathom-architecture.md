...
| **PreToolUse** (matched on `Read`, `Grep`, `Glob`, `WebFetch`, `Bash`, `mcp__*`) | Gate enforcement engine, layer 3 sub-checks | 3, 4 | Before content is fetched, checks whether the target `source_uri` is already known-inaccessible (credential/format/policy) or known-fragmented. Can `deny` with a reason, `ask` to force human confirmation, or use `updatedInput` to rewrite a query (e.g. redirect a search to the registry's authoritative source instead of a stale one). |
...
| **PostToolUseFailure** | Drift detector (explicit) | 3, 4 | A failed fetch is a first-class inaccessibility/relocation signal, not just an error. Feeds `fathomd`'s router: auth errors → layer 3 (credentials), 404/not-found → layer 4 (relocation), parse errors → layer 3 (format). |
| **PermissionDenied** | Drift detector (explicit) | 3 | Policy-driven denial, distinct from a missing credential. Routes to layer 3's policy sub-check rather than the credential sub-check. |
| **PreCompact** | `fit()` override | 2 | Claude Code's built-in compaction is the naive baseline layer 2 is meant to replace. Fathom intercepts here and, where it already holds hierarchical summaries with retrieval hooks for the content being compacted, substitutes its own summary so compaction doesn't silently drop what the retrieval-hook contract requires it to keep addressable. |
